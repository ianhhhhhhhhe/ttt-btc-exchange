...
function assignOrReadDestinationBitcoinAddress(device_address, out_noteball_address, handleBitcoinAddress){
	mutex.lock([device_address], function(device_unlock){
		db.query("SELECT to_bitcoin_address FROM note_buyer_bindings WHERE out_noteball_address=?", [out_noteball_address], function(rows){
			if (rows.length > 0){ // already know this noteball address
				device_unlock()
			...
			mutex.lock(["new_bitcoin_address"], function(unlock){
				client.getNewAddress(function(err, to_bitcoin_address, resHeaders) {
					...
					db.query(
						...
						(device_address, out_noteball_address, to_bitcoin_address) VALUES (?,?,?)", 
						[device_address, out_noteball_address, to_bitcoin_address],
						function(){
							unlock();
							device_unlock();
...
function exchangeBtcTonotes(note_buyer_deposit_id, onDone){
	if (!onDone)
		onDone = function(){};
	db.query(
		"SELECT satoshi_amount, out_noteball_address, note_buyer_bindings.device_address, confirmation_date, buy_price \n\
...
function initChat(exchangeService){
	...
	eventBus.on('text', function(from_address, text){
		...
		readCurrentState(from_address, function(state){
			...
			if (lc_text === 'buy'){
				device.sendMessageToDevice(from_address, 'text', "Buying at "+instant.getBuyRate()+" BTC/MN.  Please let me know your noteball address (just click \"...\" button and select \"Insert my address\").");
			...
			if (arrMatches){
				...
				if (price){
					readCurrentOrderPrice(from_address, order_type, function(best_price){
						...
						if (!best_price){
							response += '.\n' + (order_type === 'buy' ? "Please let me know your noteball address (just click \"...\" button and select \"Insert my address\")." : "Please let me know your Bitcoin address.");
							updateState(from_address, (order_type === 'buy') ? 'waiting_for_noteball_address' : 'waiting_for_bitcoin_address');
						}
			...
			var bValidnoteballAddress = (arrMatches && ValidationUtils.isValidAddress(arrMatches[1]));
			if (bValidnoteballAddress){ // new BB address: create or update binding
				var out_noteball_address = arrMatches[1];
				assignOrReadDestinationBitcoinAddress(from_address, out_noteball_address, function(to_bitcoin_address){
					readCurrentPrices(from_address, function(buy_price, sell_price){
						...
						device.sendMessageToDevice(from_address, 'text', "Got it, you'll receive your notes to "+out_noteball_address+".  Now please pay BTC to "+to_bitcoin_address+".  We'll exchange as much as you pay, but the "+maximum_text+" minimum is "+(MIN_SATOSHIS/1e8)+" BTC (if you send less, it'll be considered a donation).  "+will_do_text);
					});
				...
				return device.sendMessageToDevice(from_address, 'text', "This doesn't look like a valid noteball address.  Please click \"...\" button at the bottom of the screen and select \"Insert my address\", then hit \"Send\" button.");
